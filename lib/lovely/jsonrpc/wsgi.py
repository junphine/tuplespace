...
from lovely.jsonrpc import dispatcher as json_dispatcher
import logging
import threading

dispatcher_factory=threading.local()
    ...
    def handler(self, environ, start_response):
        ...
        if environ['REQUEST_METHOD'].endswith('POST'):
            ...
            if not dispatcher:
                ...
                if  hasattr(dispatcher_factory,entry_point):
                    dispatcher=dispatcher_factory.__getattribute__(entry_point)
                else:
                    factory=self.factory.get(entry_point)
                    if factory:
                        dispatcher = json_dispatcher.JSONRPCDispatcher()
                        instance=factory()
                        dispatcher.register_instance(instance)
                        dispatcher_factory.__setattr__(entry_point,dispatcher)
                if not dispatcher:  
